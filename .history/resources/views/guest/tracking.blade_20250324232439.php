...
@section('content')
                        ...
                        <div class="tracking-card">
                            ...
                            <div class="tracking-body">
                                <div class="alert alert-warning">
                                    <i class="bi bi-exclamation-triangle"></i> {{ $notFoundMessage }}
                                </div>

                                <div class="search-again-form mt-4">
                                    <h4>{{ __('messages.tracking.search_again') }}</h4>
                                    <form action="{{ route('shipment.search', ['locale' => app()->getLocale()]) }}"
                                        method="POST" class="mt-3">
                                        @csrf
                                        <div class="input-group">
                                            <input type="text" name="tracking_number" class="form-control"
                                                placeholder="{{ __('messages.hero.track_input') }}" required>
                                            <button type="submit"
                                                class="btn btn-accent">{{ __('messages.hero.track_button') }}</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="tracking-body">
                                <div class="tracking-info">
                                    ...
                                    <div class="info-row">
                                        ...
                                        <div class="info-value">{{ $shipment->current_location ?: $shipment->origin }}
                                        </div>
                                    </div>
                                    ...
                                    <div class="info-row">
                                        <div class="info-label">{{ __('messages.shipment_emails.estimated_arrival_date') }}
                                        </div>
                                        <div class="info-value">{{ $shipment->estimated_arrival_date->format('d/m/Y') }}
                                        </div>
                                    </div>
                                ...
                                @if ($shipment->payments->isNotEmpty())
                                    <div class="payment-details mt-5">
                                        ...
                                        <div class="info-row">
                                            ...
                                            <div class="info-value">{{ number_format($payment->amount, 2) }}
                                                {{ $payment->currency->symbol }}</div>
                                        </div>

                                        <div class="info-row">
                                            <div class="info-label">{{ __('messages.shipment_emails.payment_method') }}
                                            </div>
                                            <div class="info-value">
                                        ...
                                        <div class="info-row">
                                            <div class="info-label">{{ __('messages.shipment_emails.payment_status') }}
                                            </div>
                                            <div class="info-value">
                                                <span
                                                    class="status-badge payment-status-{{ $payment->status }} fit-content">
                                        ...
                                        @if ($payment->instructions)
                                            <div class="info-row">
                                                <div class="info-label">{{ __('messages.tracking.payment_instructions') }}
                                                </div>
                                                <div class="info-value formatted-text">
                                                    {!! nl2br(
                                                        preg_replace(
    ...
    /* Add styles for the search again form */
    .search-again-form {
    background-color: #f9f9f9;
    padding: 20px;
    border-radius: 8px;
