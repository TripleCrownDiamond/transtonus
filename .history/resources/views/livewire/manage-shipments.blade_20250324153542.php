    ...
    <div x-data="{
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            <div class="flex justify-between items-center">
                <h2 class="text-xl font-semibold text-gray-800">Gestion des expéditions</h2>
                <div>
                ...
                <div x-show="activeTab === 'shipment'">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            @error('locale') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                    </div>
       ...
       @endif
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($shipments as $shipment)
                        <tr class="{{ $isEditing && $shipmentId == $shipment->id ? 'bg-blue-50 border-l-4 border-blue-500' : 'hover:bg-gray-50' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @if($shipment->current_location)
                                    <div class="text-xs text-gray-500 mt-1">
                                        <span class="font-medium">Position actuelle:</span> {{ $shipment->current_location }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    <span class="font-medium">Départ:</span> {{ $shipment->departure_date->format('d/m/Y') }}
                                </div>
                                <div class="text-sm text-gray-900">
                                    <span class="font-medium">Arrivée estimée:</span> {{ $shipment->estimated_arrival_date->format('d/m/Y') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                ...
                                @endphp
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $statusClasses[$shipment->status] }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($shipment->payments->count() > 0)
                                    ...
                                    <div class="text-xs text-gray-500">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $paymentStatusClasses[$payment->status] }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                ...
                                @else
                                    <button wire:click="editShipment({{ $shipment->id }})" 
                                        ...
                                        <span wire:loading.remove wire:target="editShipment({{ $shipment->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                            </svg>
                                        </span>
                                        <span wire:loading wire:target="editShipment({{ $shipment->id }})" class="flex items-center">
                                    ...
                                    <button wire:click="showDeleteModal({{ $shipment->id }})" 
                                        ...
                                        <span wire:loading.remove wire:target="showDeleteModal({{ $shipment->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                            </svg>
                                        </span>
                                        <span wire:loading wire:target="showDeleteModal({{ $shipment->id }})" class="flex items-center">
        ...
        <div x-data="{ show: @entangle('showDeleteConfirmation') }" x-show="show" x-cloak
            class="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center" style="background-color: rgba(0, 0, 0, 0.5);">
            <div class="relative bg-white rounded-lg max-w-md w-full mx-auto p-6 shadow-xl" @click.away="show = false">
