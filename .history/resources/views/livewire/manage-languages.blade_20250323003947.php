...
<!-- Supprimer tout ce script -->
<!-- <script>
    document.addEventListener('livewire:initialized', () => {
        @this.on('showMessage', (data) => {
            const alertContainer = document.getElementById('alert-container');
            const alertBox = document.getElementById('alert-box');
            const alertTitle = document.getElementById('alert-title');
            const alertMessage = document.getElementById('alert-message');
            
            // Définir le type d'alerte (succès ou erreur)
            if (data.type === 'success') {
                alertBox.className = 'bg-green-100 border-green-400 text-green-700 border px-4 py-3 rounded relative';
                alertTitle.textContent = 'Succès! ';
            } else {
                alertBox.className = 'bg-red-100 border-red-400 text-red-700 border px-4 py-3 rounded relative';
