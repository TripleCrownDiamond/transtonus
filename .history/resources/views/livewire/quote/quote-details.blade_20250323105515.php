<div>
    ...
    <div x-data="{ open: @entangle('showQuoteDetails').live }" x-show="open" x-cloak
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300"
                ...
                <!-- Fix for the button method name -->
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button wire:click="sendEmail" 
                        wire:loading.attr="disabled" 
                        wire:target="sendEmail"
                        class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-[#00A7E1] text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#00A7E1] sm:ml-3 sm:w-auto sm:text-sm disabled:bg-indigo-300 disabled:cursor-not-allowed">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor" wire:loading.class="hidden" wire:target="sendEmail">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <svg xmlns="http://www.w3.org/2000/svg" class="animate-spin h-5 w-5 mr-2 hidden" fill="none" 
                            viewBox="0 0 24 24" stroke="currentColor" wire:loading.class.remove="hidden" wire:target="sendEmail">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                        </svg>
                        <span wire:loading.remove wire:target="sendEmail">Envoyer le devis</span>
...
<script>
    document.addEventListener('livewire:initialized', function () {
        // Initialize TinyMCE with a slight delay to ensure the element exists
        setTimeout(function() {
            if (typeof tinymce !== 'undefined' && document.getElementById('emailContent')) {
                tinymce.init({
                    ...
                    setup: function (editor) {
                        editor.on('change', function (e) {
                            window.Livewire.find(document.getElementById('emailContent').closest('[wire\\:id]').getAttribute('wire:id')).set('emailContent', editor.getContent());
                        });
                        
                        // Update TinyMCE when Livewire updates the content
                        window.Livewire.on('quoteSelected', function () {
                            if (editor) {
                                editor.setContent(window.Livewire.find(document.getElementById('emailContent').closest('[wire\\:id]').getAttribute('wire:id')).get('emailContent'));
                            }
                        });
                    }
                });
