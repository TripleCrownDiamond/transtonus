<div>
    ...
    <div x-data="{
        ...
        <div class="p-4 lg:p-6 bg-gradient-to-r from-[#00A7E1] to-[#0087C1] border-b border-gray-200 rounded-t-lg">
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center">
                    ...
                    <h2 class="text-xl font-semibold text-white">Gestion des méthodes de paiement</h2>
                </div>

                <!-- Bouton Ajouter/Fermer -->
                <div>
                    <button wire:click="toggleForm" wire:loading.attr="disabled"
                        class="px-4 py-2 rounded-md shadow-sm inline-flex items-center transition-colors duration-150 bg-white text-[#5E0035] hover:bg-gray-100 border border-transparent"
                        {{ $showForm ? 'disabled' : '' }}>
                        @if (!$showForm)
        ...
        <div x-show="formVisible" x-transition:enter="transition ease-out duration-300"
            ...
            <form wire:submit.prevent="savePaymentMethod">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="name" id="name"
                            class="mt-1 focus:ring-[#00A7E1] focus:border-[#00A7E1] block w-full shadow-sm sm:text-sm border-gray-300 rounded-md border">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" wire:model="code" id="code"
                            class="mt-1 focus:ring-[#00A7E1] focus:border-[#00A7E1] block w-full shadow-sm sm:text-sm border-gray-300 rounded-md border">
                        <p class="mt-1 text-xs text-gray-500">Identifiant unique utilisé dans le système pour
                            référencer cette méthode de paiement</p>
                        @error('code')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <div class="flex items-center">
                            <input type="checkbox" wire:model="is_active" id="is_active"
                                class="focus:ring-[#00A7E1] h-4 w-4 text-indigo-600 border-gray-300 rounded border">
                ...
                <div class="mt-6 flex justify-end">
                    <button type="button" wire:click="cancelForm" wire:loading.attr="disabled"
                        class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#00A7E1] mr-3 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center">
                    ...
                    <button type="submit" wire:loading.attr="disabled"
                        class="inline-flex items-center justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-[#00A7E1] hover:[#0087C1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#00A7E1] disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($paymentMethods as $method)
                            <tr
                                class="{{ $isEditing && $paymentMethodId == $method->id ? 'bg-blue-50 border-l-4 border-blue-500' : 'hover:bg-gray-50' }}">
                                    ...
                                    @else
                                        <button wire:click="editPaymentMethod({{ $method->id }})"
                                            ...
                                            <span wire:loading.remove
                                                wire:target="editPaymentMethod({{ $method->id }})">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                    class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <span wire:loading wire:target="editPaymentMethod({{ $method->id }})"
                                        ...
                                        <button wire:click="showDeleteModal({{ $method->id }})"
                                            ...
                                            <span wire:loading.remove
                                                wire:target="showDeleteModal({{ $method->id }})">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                    class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <span wire:loading wire:target="showDeleteModal({{ $method->id }})"
                                                class="flex items-center">
            ...
            <div class="px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                ...
                @else
                    <div class="text-sm text-gray-500 py-2">
                        Affichage de {{ $paymentMethods->count() }}
                        {{ $paymentMethods->count() > 1 ? 'méthodes de paiement' : 'méthode de paiement' }}
                    </div>
                @endif
            </div>
        </div>
        ...
        <div x-show="$wire.showDeleteConfirmation" x-cloak class="fixed z-10 inset-0 overflow-y-auto"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">
                                        Êtes-vous sûr de vouloir supprimer ce moyen de paiement? Cette action est
                                        irréversible.
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button wire:click="confirmDelete" type="button" wire:loading.attr="disabled"
                        ...
                        <button wire:click="cancelDelete" type="button" wire:loading.attr="disabled"
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#00A7E1] sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
